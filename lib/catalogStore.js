...
const { normalizeCategories, normalizePlaceCategoryFields } = require("./placeCategories");
...
function readCatalogPlaces() {
  ...
  return mergeById(demoData.places, dynamic).map((place) => normalizePlaceCategoryFields(place));
}
...
function createPlace(body) {
  ...
  const legacyCategory = String(body.category || "").trim();
  const categoryInput = body.categories !== undefined
    ? body.categories
    : legacyCategory && legacyCategory !== existing.category
      ? [legacyCategory]
      : (existing.categories ?? existing.category ?? legacyCategory);
