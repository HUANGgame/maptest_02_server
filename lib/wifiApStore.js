...
function upsertWifiAps(records) {
  ...
  const saved = records.map((record) => {
    ...
    const incomingIsOlder = parseTime(normalized.collectorUpdatedAt) < parseTime(previous.collectorUpdatedAt);
    const collectorValues = incomingIsOlder ? previous : normalized;
    const next = {
      ...previous,
      ...collectorValues,
      x: previous.positionManaged ? previous.x : collectorValues.x,
      y: previous.positionManaged ? previous.y : collectorValues.y,
...
function normalizeWifiAp(record, now) {
  ...
  return {
    ...
    category: ALLOWED_CATEGORIES.has(String(record.category || "").toLowerCase()) ? String(record.category).toLowerCase() : "unknown",
