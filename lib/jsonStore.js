...
function appendScans(records, includeExisting = false) {
  ensureDataDir();
  ...
  for (const record of records) {
    const candidate = {
      ...
      dataPriority: record.dataPriority || "",
  ...
  return includeExisting ? existing.concat(normalized).filter(record => requestedKeys.has(scanIdentityKey(record))) : normalized;
}

function deleteScansByPredicate(predicate) {
  ensureDataDir();
  const existing = readScans();
  const remaining = [];
  const deleted = [];
  ...
  if (deleted.length > 0) {
    fs.writeFileSync(scansPath, JSON.stringify(remaining, null, 2), "utf8");
  }
  return { deletedCount: deleted.length, remainingCount: remaining.length, deleted };
}
...
function normalizeKnownScanOwnership(record) {
  ...
  if (record.mapId === "k-area-airport" && (record.floorId === "k-area-airport-f1" || record.floorId === "k-area-airport-b1" || record.floorId === "floor_1" || record.floorId === "1" || record.floorId === "1F" || record.floorId === "B1" || record.floorId === "-1")) {
    record.floorId = "k-area-airport-1f";
    return true;
  }
  return false;
