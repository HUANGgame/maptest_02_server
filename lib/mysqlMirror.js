...
async function ensureSchema() {
  ...
  `);
  const [categoryColumns] = await pool.query("SHOW COLUMNS FROM places LIKE 'categoriesJson'");
  if (categoryColumns.length === 0) await pool.query("ALTER TABLE places ADD COLUMN categoriesJson TEXT AFTER category");
  await pool.query(`
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS strategy_training_runs (
      ...
      createdAt DATETIME NULL,
      updatedAt DATETIME NULL,
      INDEX idx_strategy_runs_scope (mapId, floorId),
      INDEX idx_strategy_runs_active (mapId, floorId, isActive)
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci
  `);
  await pool.query(`
...
async function seedMysqlFromJsonIfEmpty() {
  ...
  await upsertPolicyRuns(readJson("strategy_training_runs.json", []));
...
async function mirrorAdminData(snapshot = {}) {
  ...
  if (snapshot.policyRuns) await upsertPolicyRuns(snapshot.policyRuns);
...
async function upsertPlaces(places) {
  for (const place of places) {
    await pool.query(
      `INSERT INTO places
       (id, mapId, floorId, name, category, categoriesJson, x, y, description, keywords, searchable, businessStatus, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE mapId=VALUES(mapId), floorId=VALUES(floorId), name=VALUES(name),
       category=VALUES(category), categoriesJson=VALUES(categoriesJson), x=VALUES(x), y=VALUES(y), description=VALUES(description), keywords=VALUES(keywords),
      ...
      [
        ...
        place.name || place.id,
        place.category || "",
        JSON.stringify(Array.isArray(place.categories) && place.categories.length ? place.categories : [place.category || "未分類"]),
        Number(place.x || 0),
        Number(place.y || 0),
        place.description || "",
        place.keywords || "",
...
async function upsertPolicyRuns(runs) {
