...
function readRouteNodes() {
  ...
  return mergeById(routeNodes, graph.nodes).filter((node) => !deleted.has(node.id));
}
...
function readRouteEdges() {
  ...
  return mergeById(routeEdges, graph.edges)
...
function readFloorTransitions() {
  const graph = readGraph();
  const deletedNodes = new Set(graph.deletedNodeIds);
  return mergeById(floorTransitions, graph.transitions)
...
function emptyGraph() {
  return { nodes: [], edges: [], transitions: [], deletedNodeIds: [], deletedEdgeIds: [], undoStack: [] };
}
