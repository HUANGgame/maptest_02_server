...
function deleteRouteEdge(edgeId) {
  ...
  if (graph.edges.length !== before) {
    writeGraph(graph);
    return { success: true, deleted: true, mode: "delete" };
  }
  const blocked = setRouteEdgeBlocked(id, true);
  return blocked ? { success: true, deleted: false, mode: "blocked", edge: blocked } : null;
}
