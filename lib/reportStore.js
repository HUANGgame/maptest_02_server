...
function appendReport(report) {
  ...
  const existing = report.clientReportId && reports.find(item =>
    item.clientReportId === report.clientReportId && item.userId === report.userId);
...
function deleteReports(filter = {}) {
  const reports = readReports();
  const remaining = reports.filter((report) => {
    if (filter.clientReportId && report.clientReportId === filter.clientReportId) return false;
    if (filter.id && String(report.id) === String(filter.id)) return false;
    return true;
  });
  if (remaining.length === reports.length) return 0;
  writeReports(remaining);
  return reports.length - remaining.length;
}
