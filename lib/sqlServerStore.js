...
async function mirrorWifiScans(records) {
  ...
  const concurrency = Math.max(1, Math.min(Number(process.env.AZURE_SQL_WRITE_CONCURRENCY || 5), 10));
  for (let index = 0; index < records.length; index += concurrency) {
    await Promise.all(records.slice(index, index + concurrency).map(upsertWifiScan));
  }
  return true;
}
...
async function upsertWifiScan(record) {
    ...
    request.input("recordKey", sql.Char(64), scanIdentityKey(record));
    request.input("sourceId", sql.BigInt, finiteOrNull(record.id));
    request.input("sampleId", sql.NVarChar(100), textOrNull(record.sampleId));
    request.input("pointId", sql.NVarChar(100), String(record.pointId || ""));
    request.input("mapId", sql.NVarChar(100), String(record.mapId || ""));
    request.input("floorId", sql.NVarChar(100), String(record.floorId || ""));
    request.input("x", sql.Float, Number(record.x));
    request.input("y", sql.Float, Number(record.y));
    request.input("heading", sql.Float, finiteOrNull(record.heading));
    request.input("ssid", sql.NVarChar(256), textOrNull(record.ssid));
    request.input("bssid", sql.NVarChar(64), String(record.bssid || "").toLowerCase());
    request.input("rssi", sql.Int, Number(record.rssi));
    request.input("deviceInfo", sql.NVarChar(320), textOrNull(record.deviceInfo));
    request.input("sourceMode", sql.NVarChar(80), textOrNull(record.sourceMode || record.source));
    request.input("dataPriority", sql.NVarChar(40), textOrNull(record.dataPriority));
    request.input("sessionId", sql.NVarChar(100), textOrNull(record.sessionId));
    request.input("scanFreshness", sql.NVarChar(80), textOrNull(record.scanFreshness));
    request.input("scannedAt", sql.DateTime2(3), dateOrNull(record.scannedAt));
    request.input("uploadedAt", sql.DateTime2(3), dateOrNull(record.uploadedAt));
    request.input("createdAt", sql.DateTime2(3), dateOrNull(record.createdAt));
    await request.query(`
