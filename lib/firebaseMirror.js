...
async function readDocumentFiles(fileNames = DATA_FILES) {
  if (!enabled) return {};
  const documents = {};
  for (const fileName of fileNames) {
    if (!DATA_FILES.includes(fileName) || fileName === "wifi_scans.json") continue;
    const snapshot = await root().child(collectionName(fileName)).once("value");
    const remoteValue = snapshot.val();
    if (!hasMeaningfulValue(remoteValue)) continue;
    documents[fileName] = normalizeRemoteValue(fileName, remoteValue);
  }
  return documents;
}
