  ...
  return Object.values(index.points || {})
    .map((point) => {
      ...
      return {
        ...
        sampleCount: Number(point.sampleCount || point.scanCount || 0),
        stableApCount: entries.length,
        stabilityScore: entries.length ? 0.8 : 0,
        repeatBoost: Math.max(0, Math.min(3.2, Math.log(Math.max(1, Number(point.sampleCount || point.scanCount || 0))))),
        priorityWeight: 1,
        recencyWeight: 1,
      };
    })
