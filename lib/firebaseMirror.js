...
async function deleteWifiScansByPointIds(mapId, floorId, pointIds) {
  if (!enabled) throw new Error("Firebase mirror is not initialized");
  const pointIdSet = new Set(pointIds);
  let deletedCount = 0;
  for (const pointId of pointIdSet) {
    const snapshot = await root().child("wifi_scans").orderByChild("pointId").equalTo(pointId).once("value");
    const updates = {};
    snapshot.forEach((child) => {
      const record = child.val() || {};
      if (record.mapId === mapId && record.floorId === floorId && pointIdSet.has(record.pointId)) {
        updates[`wifi_scans/${child.key}`] = null;
        deletedCount += 1;
      }
    });
    const entries = Object.entries(updates);
    for (let index = 0; index < entries.length; index += WIFI_SCAN_BATCH_SIZE) {
      await root().update(Object.fromEntries(entries.slice(index, index + WIFI_SCAN_BATCH_SIZE)));
    }
  }
  if (deletedCount > 0) await writeMeta({ lastWifiCleanupAt: new Date().toISOString(), lastWifiCleanupDeletedCount: deletedCount });
  return { deletedCount };
}
