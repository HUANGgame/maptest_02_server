...
const runsPath = path.join(dataDir, "strategy_training_runs.json");
const logsPath = path.join(dataDir, "navigation_policy_logs.json");

function readPolicyRuns() {
  return readArray(runsPath);
}

function createPolicyRun(run) {
  const runs = readPolicyRuns();
  const now = new Date().toISOString();
  const next = {
    id: run.id || `policy-${Date.now()}`,
    mapId: run.mapId,
    floorId: run.floorId,
