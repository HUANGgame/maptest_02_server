...
function createRouteZone(body) {
  ...
  const graph = readGraph();
  const mergeResult = mergeZoneWithSimilarOverlaps(graph, zone);
  graph.undoStack.push(mergeResult.mergedZones.length > 0
    ? { type: "merge-zone", created: mergeResult.zone, removed: mergeResult.mergedZones }
    : { type: "create-zone", zone });
  writeGraph(graph);
  return {
    success: true,
    zone: mergeResult.zone,
    merged: mergeResult.mergedZones.length > 0,
    mergedZoneIds: mergeResult.mergedZones.map((item) => item.id),
  };
}
...
function updateRouteZone(zoneId, body) {
  ...
  const graph = readGraph();
  graph.zones = graph.zones.filter((zone) => zone.id !== id);
  const mergeResult = mergeZoneWithSimilarOverlaps(graph, next);
  graph.undoStack.push(mergeResult.mergedZones.length > 0
    ? { type: "update-merge-zone", before: existing, created: mergeResult.zone, removed: mergeResult.mergedZones }
    : { type: "update-zone", before: existing, after: next });
  writeGraph(graph);
  return {
    success: true,
    zone: mergeResult.zone,
...
function createRouteNode(body) {
  ...
  graph.undoStack.push({ type: "create-node", node });
  writeGraph(graph);
...
function createRouteSegment(body) {
  ...
  const createdNodes = [fromNode, toNode].filter((node) => node.source === "admin" && !existingNodes.some((existing) => existing.id === node.id));
  graph.undoStack.push({ type: "create-segment", edge, nodes: createdNodes });
  writeGraph(graph);
...
function restoreLastDeleted() {
  ...
  if (action.type === "create-zone") {
    graph.zones = graph.zones.filter((zone) => zone.id !== action.zone.id);
    graph.deletedZoneIds = Array.from(new Set(graph.deletedZoneIds.concat(action.zone.id)));
  }
  if (action.type === "update-zone") {
    graph.zones = mergeById(graph.zones, [action.before]);
    graph.deletedZoneIds = graph.deletedZoneIds.filter((id) => id !== action.before.id);
  }
  if (action.type === "merge-zone") {
    graph.zones = graph.zones.filter((zone) => zone.id !== action.created.id);
    graph.deletedZoneIds = Array.from(new Set(graph.deletedZoneIds.concat(action.created.id)));
    (action.removed || []).forEach((zone) => {
      graph.deletedZoneIds = graph.deletedZoneIds.filter((id) => id !== zone.id);
      graph.zones = mergeById(graph.zones, [zone]);
    });
  }
  if (action.type === "update-merge-zone") {
    graph.zones = graph.zones.filter((zone) => zone.id !== action.created.id);
    graph.deletedZoneIds = Array.from(new Set(graph.deletedZoneIds.concat(action.created.id)));
    graph.deletedZoneIds = graph.deletedZoneIds.filter((id) => id !== action.before.id);
    graph.zones = mergeById(graph.zones, [action.before]);
  ...
  if (action.type === "create-node") {
    graph.nodes = graph.nodes.filter((node) => node.id !== action.node.id);
    graph.deletedNodeIds = Array.from(new Set(graph.deletedNodeIds.concat(action.node.id)));
  }
  if (action.type === "create-segment") {
    graph.edges = graph.edges.filter((edge) => edge.id !== action.edge.id);
    graph.deletedEdgeIds = Array.from(new Set(graph.deletedEdgeIds.concat(action.edge.id)));
    (action.nodes || []).forEach((node) => {
      graph.nodes = graph.nodes.filter((item) => item.id !== node.id);
      graph.deletedNodeIds = Array.from(new Set(graph.deletedNodeIds.concat(node.id)));
    });
  }
  if (action.type === "create-transition") {
    graph.transitions = graph.transitions.filter((transition) => !(action.transitions || []).some((item) => item.id === transition.id));
  }
...
function createFloorTransition(body) {
  ...
  graph.undoStack.push({ type: "create-transition", transitions: records });
  writeGraph(graph);
...
function mergeZoneWithSimilarOverlaps(graph, zone) {
  const activeDeleted = new Set(graph.deletedZoneIds);
  const candidates = graph.zones.filter((item) => {
    if (activeDeleted.has(item.id)) return false;
    if (item.id === zone.id) return false;
    if (item.mapId !== zone.mapId || item.floorId !== zone.floorId) return false;
    if (item.zoneType !== zone.zoneType) return false;
    return zonesAreNearDuplicates(item, zone);
  });
  if (candidates.length === 0) {
    graph.zones = mergeById(graph.zones, [zone]);
    graph.deletedZoneIds = graph.deletedZoneIds.filter((id) => id !== zone.id);
    return { zone, mergedZones: [] };
  }

  const merged = normalizeZone({
    ...largestZoneByArea([zone].concat(candidates)),
    id: safeId(zone.id),
    label: zone.label || candidates[0].label || zoneTypeLabel(zone.zoneType),
    updatedAt: new Date().toISOString(),
  });
  graph.zones = graph.zones.filter((item) => !candidates.some((candidate) => candidate.id === item.id) && item.id !== zone.id);
  graph.zones = mergeById(graph.zones, [merged]);
  graph.deletedZoneIds = graph.deletedZoneIds.filter((id) => id !== merged.id);
  graph.deletedZoneIds = Array.from(new Set(graph.deletedZoneIds.concat(candidates.map((item) => item.id))));
  return { zone: merged, mergedZones: candidates };
}

function zonesAreNearDuplicates(leftZone, rightZone) {
  const left = zoneRect(leftZone);
  const right = zoneRect(rightZone);
  const overlap = rectOverlapArea(left, right);
...
function largestZoneByArea(zones) {
  return zones
    .map((zone) => ({ zone, area: rectArea(zoneRect(zone)) }))
    .sort((left, right) => right.area - left.area)[0].zone;
}

function zonesOverlapOrTouch(leftZone, rightZone, tolerance = 0) {
  const left = zoneRect(leftZone);
  const right = zoneRect(rightZone);
  return left.left <= right.right + tolerance &&
    left.right + tolerance >= right.left &&
    left.top <= right.bottom + tolerance &&
    left.bottom + tolerance >= right.top;
}

function boundsToZone(zones) {
